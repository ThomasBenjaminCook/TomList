    ...
    if(len(recipes_indicies) > 0):
        ...
        for instruction in recipes_dataframe["instructions"]:
            ...
            if(recipes_dataframe.loc[recipe_index,"is_edit"] == "two"):
                ...
                recipe_string = recipe_string + '<div class="image-upload"><label for="file-input"><img src="{{url_for(' + "'static', filename='default.jpg')}}"+'" width=25%/></label><input type="file" id="file-input" name="'+personal_remove_id+"_image"+'">$("input[name=\'file_1\']").change(function() { this.form.submit(); });</div></br>'
