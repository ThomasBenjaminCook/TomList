    ...
    if(len(recipes_indicies) > 0):
        ...
        for instruction in recipes_dataframe["instructions"]:
            ...
            if(recipes_dataframe.loc[recipe_index,"is_edit"] == "two"):
                ...
                recipe_string = recipe_string + '<div class="image-upload"><label for="file-input"><img src="{{url_for(' + "'static', filename='default.jpg')}}"+'" width=25%/></label><form method="POST" enctype="multipart/form-data"><input type="file" id="file-input" name="file_1"></form></div></br>'
